...
const primaryButtonClass =
    'inline-flex items-center rounded-md bg-indigo-600 dark:bg-indigo-500 px-4 py-2 text-xs font-semibold text-white shadow-sm hover:bg-indigo-500 dark:hover:bg-indigo-400 disabled:opacity-50 transition';

const tinyPrimaryButtonClass =
    'px-3 py-1 text-[11px] rounded-md bg-indigo-600 dark:bg-indigo-500 text-white font-semibold shadow-sm hover:bg-indigo-500 dark:hover:bg-indigo-400 disabled:opacity-50 transition';
...
const badgePlatformClass =
    'px-2 py-0.5 rounded-full text-[10px] font-medium bg-indigo-100 dark:bg-indigo-900/60 text-indigo-700 dark:text-indigo-200 border border-indigo-200 dark:border-indigo-500/40';
</script>
...
<template>
    ...
    <AuthenticatedLayout>
        <div class="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100">
            ...
            <main class="w-[95%] mx-auto pt-8 pb-12 space-y-8">
                ...
                <section :class="bigCardClass">
                    <div class="text-slate-900 dark:text-slate-50 space-y-6 text-sm">
                        ...
                        <form @submit.prevent="createSource" class="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
                            ...
                            <div class="md:col-span-1">
                                ...
                                <select v-model="createForm.campaign_id"
                                    class="block w-full rounded-md border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 text-sm text-slate-900 dark:text-slate-100 shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                    <option v-for="campaign in campaigns.data" :key="campaign.id" :value="campaign.id">
                                        {{ campaign.name }}
                                    </option>
                                </select>
                                <div v-if="createForm.errors.campaign_id" class="text-xs text-red-500 dark:text-red-400 mt-1">
                            ...
                            <div class="md:col-span-1">
                                ...
                                <div v-if="createForm.errors.platform" class="text-xs text-red-500 dark:text-red-400 mt-1">
                ...
                <section :class="bigCardClass">
                    <div class="text-slate-900 dark:text-slate-50 text-sm">
                        ...
                        <div v-else class="space-y-4">
                            <div v-for="campaign in campaigns.data" :key="campaign.id"
                                class="border border-slate-200 dark:border-slate-800 rounded-lg p-4 bg-slate-50 dark:bg-slate-950/50">
                                ...
                                <div v-if="!campaign.sources.length" class="text-xs text-slate-500 dark:text-slate-400">
                                    Aucune source pour cette campagne.
                                </div>

                                <div v-else class="mt-2 space-y-2">
                                    <div v-for="source in campaign.sources" :key="source.id"
                                        class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 text-xs bg-white dark:bg-slate-900 border border-slate-200 dark:border-transparent rounded-md px-3 py-3">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <div v-if="source.notes" class="text-xs text-slate-500 dark:text-slate-400 mt-1">
                                                {{ source.notes }}
                                            </div>
                                            ...
                                            >
                                                <div
                                                    ...
                                                    :key="tracked.id"
                                                    class="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 px-3 py-3 text-xs text-slate-900 dark:text-slate-100 space-y-2 w-full sm:max-w-2xl"
                                                >
                                                    <div class="flex flex-col gap-1">
                                                        <p class="text-[11px] font-semibold text-slate-500 dark:text-slate-300 uppercase tracking-[0.2em]">
                                                        ...
                                                        <p class="font-mono text-slate-900 dark:text-slate-50 text-sm truncate">
                                                            {{ trackingUrl(tracked) }}
                                                        </p>
                                                    ...
                                                    <div class="flex justify-end">
                                                        <button
                                                            type="button"
                                                            class="inline-flex items-center gap-2 rounded-md border border-indigo-500 px-3 py-1 text-[11px] font-semibold text-indigo-600 dark:text-indigo-200 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition"
                                            ...
                                            <div class="mt-3 space-y-1 text-[11px]">
                                                ...
                                                <div class="flex flex-col sm:flex-row sm:items-center gap-2">
                                                    <select v-model="selectedLinks[source.id]"
                                                        class="flex-1 max-w-xs sm:max-w-sm lg:max-w-md rounded-md border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 text-xs text-slate-900 dark:text-slate-100">
                                                        <option value="">
                                                            Choisir un lien…
                                                        </option>
        ...
        <transition name="fade">
            <div
                v-if="showToast"
                class="fixed bottom-4 right-4 z-50 px-4 py-2 rounded-md shadow-lg bg-slate-900 dark:bg-slate-950 border border-slate-700 text-white text-xs flex items-center gap-2"
            >
        ...
        <!-- Modale d'édition -->
        <div v-if="isEditOpen && editingSource" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div class="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl shadow-slate-900/20 dark:shadow-slate-950/60 w-full max-w-lg">
                <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
                    <h3 class="text-sm font-semibold text-slate-900 dark:text-slate-50">
                        Éditer la source
                    </h3>
                    <button type="button" class="text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200" @click="closeEditModal">
                        ✕
                    </button>
                </div>

                <div class="px-6 py-4 space-y-4 text-sm text-slate-900 dark:text-slate-50">
                    <div>
                        <label class="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">
                            Nom de la source
                        </label>
                    ...
                    <div>
                        ...
                        <input v-model="editForm.platform" type="text"
                            class="mt-1 block w-full rounded-md border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm text-slate-900 dark:text-slate-100 shadow-sm focus:ring-indigo-500 focus:border-indigo-500" />
                        <div v-if="editForm.errors.platform" class="text-xs text-red-500 dark:text-red-400 mt-1">
